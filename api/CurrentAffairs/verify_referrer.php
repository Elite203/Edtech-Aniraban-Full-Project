...
try {
    if (!isset($_GET['referrer_id'])) {
        throw new Exception("referrer_id is required");
    }
    ...
    // 3. Check if referrer has an active Premium Monthly Test subscription
    $stmt_sub = $pdo->prepare("SELECT id FROM monthly_test_purchases WHERE student_id = ? AND expiry_date >= ? AND status = 'active' LIMIT 1");
    $stmt_sub->execute([$referrer_id, $today]);
    ...
    if ($student_id) {
        // 4. Check if referee already has an active Premium Monthly Test subscription (existing customers cannot be referred)
        $stmt_referee_sub = $pdo->prepare("SELECT id FROM monthly_test_purchases WHERE student_id = ? AND expiry_date >= ? AND status = 'active' LIMIT 1");
        $stmt_referee_sub->execute([$student_id, $today]);
        ...
        // 5. Check if a cross-referral relationship already exists
