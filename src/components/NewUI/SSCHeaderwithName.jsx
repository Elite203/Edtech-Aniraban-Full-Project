...
const SSCHeader = ({ user }) => {
    return (
        <>
            ...
            {/* Added 'gap-x-6' to parent */}
            <div className="h-8 bg-[#0047ab] w-full flex-shrink-0 relative z-10 flex items-center text-white font-bold text-sm gap-x-6">
                <span>Roll No : {user.number}</span>
                <span>|</span>
                <span>Name : {user.name || `${user.first_name || ""} ${user.last_name || ""}`.trim() || "User"}</span>
            </div>
        </>
    );
};
